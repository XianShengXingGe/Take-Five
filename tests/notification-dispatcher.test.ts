...
describe('NotificationDispatcher', () => {
  ...
  it('automatically resolves project name when project is not explicitly provided in input', async () => {
    const { dispatcher, configManager } = createDispatcher('zh-CN');
    ...
    const result = await dispatcher.dispatch({
      agent: 'claude',
      type: 'task_completed',
      cwd: '/path/to/my-auto-project',
    });

    expect(result.status).toBe('dispatched');
    expect(result.payload?.subtitle).toContain('my-auto-project');
  });

  it('correctly processes dispatchFromHook with parsed toolCall and workspace paths', async () => {
    const { dispatcher, configManager } = createDispatcher('zh-CN');
    await configManager.saveConfig(createMockConfig({ language: 'zh-CN' }));

    const rawStdin = JSON.stringify({
      toolCall: { name: 'ask_question' },
      workspacePaths: ['/Users/dev/workspace/MyAwesomeHookApp'],
    });

    const result = await dispatcher.dispatchFromHook(rawStdin, {
      agent: 'antigravity',
      type: 'task_completed',
    });

    expect(result.status).toBe('dispatched');
    expect(result.isPreToolUse).toBe(true);
    expect(result.decision).toBe('ask');
    expect(result.payload?.title).toBe('⏳ 等待输入');
    expect(result.payload?.subtitle).toContain('MyAwesomeHookApp');
  });

  it('skips dispatch and returns skipped status when hook payload signals shouldSkip', async () => {
    const { dispatcher, configManager } = createDispatcher('zh-CN');
    await configManager.saveConfig(createMockConfig());
