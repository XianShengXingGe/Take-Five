...
describe('ProjectDetector', () => {
  ...
  it('normalizes full directory paths passed in explicitProject to clean project names', () => {
    // POSIX path without git
    expect(detectProjectName({ explicitProject: '/Users/gd/Desktop/my-cool-app' })).toBe('my-cool-app');
    expect(detectProjectName({ explicitProject: '/Users/gd/Library/Mobile Documents/com~apple~CloudDocs/04 工作室/片刻' })).toBe('片刻');

    // Windows path without git
    expect(detectProjectName({ explicitProject: 'C:\\Users\\admin\\Projects\\awesome-tool' })).toBe('awesome-tool');
    expect(detectProjectName({ explicitProject: 'D:/repos/website/' })).toBe('website');
  });

  it('extracts git root name when explicitProject is a path inside a git repository', () => {
    const baseTemp = mkdtempSync(join(tmpdir(), 'takefive-explicit-git-'));
    const gitRepoDir = join(baseTemp, 'my-git-project');
    const nestedSubDir = join(gitRepoDir, 'packages', 'web');

    try {
      mkdirSync(join(gitRepoDir, '.git'), { recursive: true });
      mkdirSync(nestedSubDir, { recursive: true });

      const project = detectProjectName({ explicitProject: nestedSubDir });
      expect(project).toBe('my-git-project');
    } finally {
      rmSync(baseTemp, { recursive: true, force: true });
    }
  });
