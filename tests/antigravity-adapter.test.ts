...
describe('AntigravityAdapter', () => {
  ...
  it('injects hooks into empty / new config and detects hook status', async () => {
    ...
    expect(postStatus.hooks.task_completed).toBe('Stop (maps error termination to task_failed)');
    expect(postStatus.hooks.waiting_input).toBe('PreToolUse: ask_question');
    expect(postStatus.hooks.waiting_permission).toBe('PreToolUse: ask_permission');
    expect(postStatus.hooks.task_failed).toBe('Stop: terminationReason=error');
  });
  ...
  it('preserves existing user configuration and unrelated custom hooks', async () => {
    ...
    expect(updatedContent.takefive.Stop[0].command).toContain('notify --agent antigravity');
  });
  ...
  it('restores pre-existing configuration on uninstall via backup', async () => {
    ...
    // If config was deleted, uninstall recovers from backup
    rmSync(hooksPath);
  ...
  it('maps lifecycle events to unified types accurately', () => {
    ...
    expect(adapter.mapLifecycleEvent('request_user_input')).toBe('waiting_input');
    expect(adapter.mapLifecycleEvent('requestuserinput')).toBe('waiting_input');
  ...
  it('parses hook payload with request_user_input to waiting_input', () => {
    const payload = {
      toolCall: {
      ...
      workspacePaths: ['/Users/dev/my-antigravity-app'],
    };

    const parsed = adapter.parseHookPayload(payload, 'task_completed');
    expect(parsed.eventType).toBe('waiting_input');
    expect(parsed.reason).toBe('Do you want to proceed with database migration?');
    expect(parsed.projectCwd).toBe('/Users/dev/my-antigravity-app');
  ...
  describe('translateAntigravityEvent', () => {
    ...
    it('translates PreToolUse for request_user_input to waiting_input', () => {
      const event = translateAntigravityEvent({
        toolCall: {
          name: 'request_user_input',
          args: {
            prompt: 'Please provide credentials for staging deployment',
          },
        },
        workspacePaths: ['/Users/developer/projects/infra-deploy'],
      });

      expect(event.agent).toBe('antigravity');
      expect(event.type).toBe('waiting_input');
      expect(event.reason).toBe('Please provide credentials for staging deployment');
      expect(event.project).toBe('infra-deploy');
    });
